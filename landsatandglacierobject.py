...
def ee_download(glacierID, glacierObject, begDate = '1984-01-01', endDate = '2019-01-01', cloud_tol = 15, landsat = True, dem = True, outputObject = True):
    ...
    #Modules needed import, csv and pydrive for sending glacier object to drive
    #ee for download
    ...
    import os
    import ee
    #Initial earth engine connection, key much be on your computer, thus you must once in terminal run ee.Authenticate()
    #for any new computer that you are using for implementation of this google earth engine communication
    try:
        ee.Initialize()
    ...
    #Our glacier region can be found in the imported dictionary as an argument under bounding box (list of lists of coordinates)
    ...
    if date.fromisoformat(endDate) > date.fromisoformat("2013-01-01"):
        ...
        #In order to collect dates for object and pushing images to drive make our collection a list thus we can loop over
    ...
    #For each image collection we need the list of dates from that image as well as the sensor(to be added) it comes from, the following code extracts that information and adds to glacierObject
    ...
    # However that is where the help from developers group stopped helping, do not use it for creating folders and such, their solutions don't work with python API
    #Or are outdated
    g_login = GoogleAuth()
